...
function nextDay(dayOfTheWeek,week){
  ...
  var date = date.getFullYear()+'-'+date.getDate()+'-'+(date.getMonth()+1);
  return date
}
...
bot.command("test", ctx =>{
    ...
    bot.telegram.sendMessage(ctx.chat.id, startMainMenu, 
    {
        reply_markup:{
            inline_keyboard: [  
                [
                  { text: `Semaine : ${currentWeekNumber(0)}`, callback_data : `week-${currentWeekNumber(0)}`}
                ],
                [
                  { text: `Semaine : ${currentWeekNumber(1)}`, callback_data : `week-${currentWeekNumber(1)}`}
                ],
                [
                  { text: `Semaine : ${currentWeekNumber(2)}`, callback_data : `week-${currentWeekNumber(2)}`}
                ],
                [
                  { text: `Semaine : ${currentWeekNumber(3)}`, callback_data : `week-${currentWeekNumber(3)}`}
                ],
                [
...
bot.action(/^week-(.*)/, async (ctx) => {
  ...
  bot.telegram.sendMessage(ctx.chat.id, startMainMenu, 
  {
    reply_markup:{
        inline_keyboard: [
            [
              { text: `Lundi : ${nextDay(0,selectedWeekNumber)}`, callback_data : "date1"}
            ],
            [
            ...
            [
              { text: `Mardi : ${nextDay(1,selectedWeekNumber)}`, callback_data : "date2"}
            ],
            [
            ...
            [
              { text: `Mercredi : ${nextDay(2,selectedWeekNumber)}`, callback_data : "date3"}
            ],
            [
            ...
            [
              { text: `Jeudi : ${nextDay(3,selectedWeekNumber)}`, callback_data : "date4"}
            ],
            [
            ...
            [
              { text: `Vendredi : ${nextDay(4,selectedWeekNumber)}`, callback_data : "date5"}
            ],
            [
